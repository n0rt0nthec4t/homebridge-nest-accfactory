...
export default class NexusStreamer {
    ...
    constructor(deviceData, options) {
        ...
        if (typeof options === 'object') {
            ...
            if (typeof options?.resourcePath === 'string' &&
    ...
    startLiveStream(sessionID, videoStream, audioStream, alignToSPSFrame) {
        ...
        videoStream && videoStream.on('error', () => {
            // EPIPE errors??
        });
        audioStream && audioStream.on('error', () => {
            // EPIPE errors??
        });
    ...
    #sendMessage(type, data) {
        if (this.nexusTalk.socket === null || this.nexusTalk.socket.readyState !== 'open' ||
    ...
    #handleRedirect(payload) {
        ...
        if (typeof payload === 'object') {
            ...
            let packet = payload.readFields((tag, obj, protoBuf) => {
                if (tag === 1) {
    ...
    #handlePlaybackBegin(payload) {
        ...
        let packet = payload.readFields((tag, obj, protoBuf) => {
            ...
            if (tag === 2) {
                obj.channels.push(protoBuf.readFields((tag, obj, protoBuf) => {
                    if (tag === 1) {
        ...
        packet.channels && packet.channels.forEach((stream) => {
            ...
            if (stream.codec_type === CodecType.H264) {
                this.nexusTalk.video = {channel_id: stream.channel_id, start_time: (stream.start_time * 1000), sample_rate: stream.sample_rate, packet_time: (stream.start_time * 1000)};
            }
            if (stream.codec_type === CodecType.AAC || stream.codec_type === CodecType.OPUS || stream.codec_type === CodecType.SPEEX) {
                this.nexusTalk.audio = {channel_id: stream.channel_id, start_time: (stream.start_time * 1000), sample_rate: stream.sample_rate, packet_time: (stream.start_time * 1000)};
            }
        });
    ...
    #handlePlaybackPacket(payload) {
        ...
        let packet = payload.readFields((tag, obj, protoBuf) => {
            ...
            if (tag === 7) {
                obj.directors_cut_regions.push(protoBuf.readFields((tag, obj, protoBuf) => {
                    if (tag === 1) {
