...
function adjustTemperature(temperature, currentTemperatureUnit, targetTemperatureUnit, round) {
  ...
  // Validate supported temperature units
  if (['C', 'F'].includes(currentTemperatureUnit) === false || ['C', 'F'].includes(targetTemperatureUnit) === false) {
    return undefined;
  }
  ...
  if (currentTemperatureUnit === 'C' && targetTemperatureUnit === 'F') {
    temperature = (temperature * 9) / 5 + 32;
  }

  // Apply HomeKit-friendly rounding rules
  if (round === true) {
    // HomeKit commonly uses 0.5°C precision and whole °F values
...
function crc24(valueToHash) {
  // Validate input
  if (valueToHash === undefined || valueToHash === null) {
    return undefined;
  }

  let crc = 0xb704ce;
  let buffer = Buffer.from(String(valueToHash));

  for (let i = 0; i < buffer.length; i++) {
    let index = ((crc >> 16) ^ buffer[i]) & 0xff;
    crc = (CRC24_HASH_TABLE[index] ^ (crc << 8)) & 0xffffff;
  }
...
function parseDurationToSeconds(inputDuration, { defaultValue = null, min = 0, max = Infinity } = {}) {
  ...
  if (inputDuration !== undefined && inputDuration !== null && inputDuration !== '') {
    ...
    if (inputDuration.startsWith('-') === true) {
      return defaultValue;
    }
    ...
    // Apply optional bounds checking
    if (Number.isFinite(normalisedSeconds) === true) {
