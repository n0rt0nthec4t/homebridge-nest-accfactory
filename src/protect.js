...
export default class NestProtect extends HomeKitDevice {
    ...
    updateServices(deviceData) {
        ...
        this.batteryService.updateCharacteristic(this.hap.Characteristic.StatusLowBattery, (deviceData.battery_level > LOWBATTERYLEVEL && deviceData.battery_health_state === 0) ? this.hap.Characteristic.StatusLowBattery.BATTERY_LEVEL_NORMAL : this.hap.Characteristic.StatusLowBattery.BATTERY_LEVEL_LOW);

        // Update smoke details
        this.smokeService.updateCharacteristic(this.hap.Characteristic.StatusActive, (deviceData.online === true && deviceData.removed_from_base === false));  // If Nest isn't online or removed from base, report in HomeKit
        this.smokeService.updateCharacteristic(this.hap.Characteristic.StatusFault, ((deviceData.online === true && deviceData.removed_from_base === false) && (Math.floor(Date.now() / 1000) <= deviceData.replacement_date) ? this.hap.Characteristic.StatusFault.NO_FAULT : this.hap.Characteristic.StatusFault.GENERAL_FAULT));  // General fault if replacement date past or Nest isn't online or removed from base
        this.smokeService.updateCharacteristic(this.hap.Characteristic.SmokeDetected, (deviceData.smoke_status === 2 ? this.hap.Characteristic.SmokeDetected.SMOKE_DETECTED : this.hap.Characteristic.SmokeDetected.SMOKE_NOT_DETECTED));

        // Update carbon monoxide details
        this.carbonMonoxideService.updateCharacteristic(this.hap.Characteristic.StatusActive, (deviceData.online === true && deviceData.removed_from_base === false));  // If Nest isn't online or removed from base, report in HomeKit
        this.carbonMonoxideService.updateCharacteristic(this.hap.Characteristic.StatusFault, ((deviceData.online === true && deviceData.removed_from_base === false) && (Math.floor(Date.now() / 1000) <= deviceData.replacement_date) ? this.hap.Characteristic.StatusFault.NO_FAULT : this.hap.Characteristic.StatusFault.GENERAL_FAULT));  // General fault if replacement date past or Nest isn't online or removed from base
        this.carbonMonoxideService.updateCharacteristic(this.hap.Characteristic.CarbonMonoxideDetected, (deviceData.co_status === 2 ? this.hap.Characteristic.CarbonMonoxideDetected.CO_LEVELS_ABNORMAL : this.hap.Characteristic.CarbonMonoxideDetected.CO_LEVELS_NORMAL));

        // Update motion service if present
        if (this.motionService !== undefined) {
            // Motion detect if auto_away = false. Not supported on battery powered Nest Protects
            this.motionService.updateCharacteristic(this.hap.Characteristic.StatusActive, (deviceData.online === true && deviceData.removed_from_base === false));  // If Nest isn't online or removed from base, report in HomeKit
            this.motionService.updateCharacteristic(this.hap.Characteristic.StatusFault, ((deviceData.online === true && deviceData.removed_from_base === false) && (Math.floor(Date.now() / 1000) <= deviceData.replacement_date) ? this.hap.Characteristic.StatusFault.NO_FAULT : this.hap.Characteristic.StatusFault.GENERAL_FAULT));  // General fault if replacement date past or Nest isn't online or removed from base
