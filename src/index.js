...
export class NestAccfactory {
    ...
    constructor(log, config, api) {
        ...
        this.api.on(Homebridge.APIEvent.DID_FINISH_LAUNCHING, async () => {
        ...
        this.api.on(Homebridge.APIEvent.SHUTDOWN, async () => {
    ...
    async #connect() {
        ...
        if (typeof this.config?.nest?.access_token === 'string' && this.config?.nest?.access_token !== '') {
            ...
            await axios(request).then(async (response) => {
                ...
                let tokenExpire = Math.floor(Date.now() / 1000) + (3600 * 24);  // 24hrs expiry from now
    ...
    async #subscribeProtobuf(connectionType) {
        ...
        if (fs.existsSync(path.dirname(fileURLToPath(import.meta.url)) + '/protobuf/root.proto') === true) {
            protobuf.util.Long = null;
            protobuf.configure();
            this.#connections[connectionType].protobufRoot = protobuf.loadSync(path.dirname(fileURLToPath(import.meta.url)) + '/protobuf/root.proto');
            ...
            Object.values(deviceChanges).filter((object) => object.change === 'add').forEach((object) => {
                if (object.object_key === deviceData.uuid && deviceData.excluded === false) {
                    ...
                    if (deviceData.device_type === NestAccfactory.DeviceType.THERMOSTAT && typeof NestThermostat === 'function') {
                        ...
                        tempDevice.add('Nest Thermostat', Homebridge.Categories.THERMOSTAT, true);
                    }
                    ...
                    if (deviceData.device_type === NestAccfactory.DeviceType.TEMPSENSOR && typeof NestTemperatureSensor === 'function') {
                        ...
                        tempDevice.add('Nest Temperature Sensor', Homebridge.Categories.SENSOR, true);
                    }
                    ...
                    if (deviceData.device_type === NestAccfactory.DeviceType.SMOKESENSOR && typeof NestProtect === 'function') {
                        ...
                        tempDevice.add('Nest Protect', Homebridge.Categories.SENSOR, true);
                    }
                    ...
                    if ((deviceData.device_type === NestAccfactory.DeviceType.CAMERA ||
                        ...
                        if (deviceData.device_type === NestAccfactory.DeviceType.CAMERA) {
                            ...
                            tempDevice.add(accessoryName, Homebridge.Categories.IP_CAMERA, true);
                        }
                        if (deviceData.device_type === NestAccfactory.DeviceType.DOORBELL) {
                            ...
                            tempDevice.add(accessoryName, Homebridge.Categories.VIDEO_DOORBELL, true);
                        }
                    ...
                    if (deviceData.device_type === NestAccfactory.DeviceType.WEATHER && typeof NestWeather === 'function') {
                        ...
                        tempDevice.add('Nest Weather', Homebridge.Categories.SENSOR, true);
    ...
    #processData(connectionType, deviceUUID) {
        ...
        Object.entries(this.#rawData).filter(([key, value]) => (key.startsWith('device.') === true || (key.startsWith('DEVICE_') === true && PROTOBUF_THERMOSTAT_RESOURCES.includes(value.value?.device_info?.typeName) === true)) && (deviceUUID === '' || deviceUUID === key)).forEach(([object_key, value]) => {
            ...
            if (Object.entries(tempDevice).length !== 0 && typeof devices[tempDevice.serial_number] === 'undefined') {
                ...
                tempDevice.humiditySensor = (this.config.options.humiditySensor === true);   // Config option for seperate humidity sensorr
                tempDevice.externalCool = (typeof this.config.options?.externalCool === 'object' ? this.config.options?.externalCool : undefined); // Config option for external cooling source
                tempDevice.externalHeat = (typeof this.config.options?.externalHeat === 'object' ? this.config.options?.externalHeat : undefined); // Config option for external heating source
                tempDevice.externalFan = (typeof this.config.options?.externalFan === 'object' ? this.config.options?.externalFan: undefined); // Config option for external fan source
                tempDevice.externalDehumidifier = (typeof this.config.options?.externalDehumidifier === 'object' ? this.config.options?.externalDehumidifier : undefined);// Config option for external dehumidifier source
        ...
        Object.entries(this.#rawData).filter(([key, value]) => (key.startsWith('quartz.') === true || (key.startsWith('DEVICE_') === true && PROTOBUF_CAMERA_DOORBELL_RESOURCES.includes(value.value?.device_info?.typeName) === true)) && (deviceUUID === '' || deviceUUID === key)).forEach(([object_key, value]) => {
            ...
            */
            ...
            if (Object.entries(tempDevice).length !== 0 && typeof devices[tempDevice.serial_number] === 'undefined') {
                // Insert any extra options we've read in from configuration file for this device
                tempDevice.eveApp = (this.config.options.eveApp === true);   // Config option for EveHome App integration
                tempDevice.hksv = (this.config.options.hksv === true);   // Config option for HomeKit Secure Video
                ...
                tempDevice.chimeSwitch = (this.config.options.chimeSwitch === true);   // Config option for chime switch
                devices[tempDevice.serial_number] = tempDevice;  // Store processed device
            }
        });
    ...
    async #getWeatherData(connectionType, deviceUUID, latitude, longitude) {
        ...
        await axios(request).then((response) => {
            ...
            if (typeof response.data[latitude + ',' + longitude].current === 'object') {
                ...
                weatherData.elevation = this.config.options.elevation;  // Elevation from configuration
            }
        }).catch(() => {
