...
export default class NestCamera extends HomeKitDevice {
    ...
    addServices() {
        ...
        this.controller = new this.hap.CameraController(NestCamera.controllerOptions);
    ...
    async *handleRecordingStreamRequest(HKSVRecordingStreamID) {
        ...
        let includeAudio = (this.deviceData.audio_enabled === true && this.controller.recordingManagement.recordingManagementService.getCharacteristic(this.hap.Characteristic.RecordingAudioActive).value === this.hap.Characteristic.RecordingAudioActive.ENABLE);
        let recordCodec = VideoCodecs.LIBX264;

        // Build our ffmpeg commandline for the video stream
        let commandLine = '-hide_banner -nostats'
        ...
        commandLine = commandLine
            + ' -pix_fmt yuv420p'
            + ' -level:v ' + ((this.HKSVRecordingConfiguration.videoCodec.parameters.level === this.hap.H264Level.LEVEL4_0) ? '4.0' : (this.HKSVRecordingConfiguration.videoCodec.parameters.level === this.hap.H264Level.LEVEL3_2) ? '3.2' : '3.1')
            + ' -preset veryfast'
        ...
        if (includeAudio === true) {
            ...
            commandLine = commandLine
                ...
                + ' -profile:a ' + (this.HKSVRecordingConfiguration.audioCodec.type === this.hap.AudioRecordingCodecType.AAC_LC ? 'aac_low' : 'acc_eld')
        ...
        this.log.info('Recording started from "%s" %s %s', this.deviceData.description, (includeAudio === true ? 'with audio' : 'without audio'), (recordCodec !== VideoCodecs.COPY ? 'using H264 encoder ' + recordCodec : ''));
    ...
    closeRecordingStream(HKSVRecordingStreamID, closeReason) {
        ...
        if (closeReason === this.hap.HDSProtocolSpecificErrorReason.NORMAL) {
            ...
            this.log.warn('Recording completed with error from "%s". Reason was "%s"', this.deviceData.description, this.hap.HDSProtocolSpecificErrorReason[closeReason]);
        }
    }
    ...
    updateRecordingActive(enableHKSVRecordings) {
        ...
        if (enableHKSVRecordings === this.HKSVRecorder.record ||
            this.NexusStreamer === undefined) {
    ...
    async handleSnapshotRequest(snapshotRequestDetails, callback) {
        ...
        if (this.deviceData.streaming_enabled === true && this.deviceData.online === true) {
            if (this.deviceData.hksv === false && this.snapshotEvent.type !== '' && this.snapshotEvent.done === false) {
                ...
                let request = {
                    method: 'get',
                    url: this.deviceData.nexus_api_http_server_url + '/event_snapshot/' + this.deviceData.uuid.split('.')[1] + '/' + this.snapshotEvent.id + '?crop_type=timeline&width=' + snapshotRequestDetails.width + '&cachebuster=' + Math.floor(Date.now() / 1000),
                    headers: {
            ...
            if (this.deviceData.hksv === true || imageBuffer === undefined) {
                ...
                let request = {
                    method: 'get',
                    url: this.deviceData.nexus_api_http_server_url + '/get_image?uuid=' + this.deviceData.uuid.split('.')[1] + '&width=' + snapshotRequestDetails.width,
                    headers: {
                        'User-Agent': USERAGENT,
                        'accept' : '*/*',
                        [this.deviceData.apiAccess.key] : this.deviceData.apiAccess.value + this.deviceData.apiAccess.token,
        ...
        if (this.deviceData.online === false &&
            this.cameraOfflineImage !== undefined) {
    ...
    async handleStreamRequest(request, callback) {
        ...
        if (request.type === this.hap.StreamRequestTypes.START) {
            ...
            commandLine = commandLine
                ...
                + ' -f rtp'
                + ' -srtp_out_suite ' + this.hap.SRTPCryptoSuites[this.ongoingSessions[request.sessionID].videoCryptoSuite] + ' -srtp_out_params ' + this.ongoingSessions[request.sessionID].videoSRTP.toString('base64')
            ...
            if (this.deviceData.audio_enabled === true) {
                commandLine = commandLine
                    ...
                    + ' -f rtp'
                    + ' -srtp_out_suite ' + this.hap.SRTPCryptoSuites[this.ongoingSessions[request.sessionID].audioCryptoSuite] + ' -srtp_out_params ' + this.ongoingSessions[request.sessionID].audioSRTP.toString('base64')
            ...
            if (this.deviceData.audio_enabled === true &&
                ...
                ffmpegAudioTalkback.stdin.write('v=0\n'
                    ...
                    + 'a=crypto:1 ' + this.hap.SRTPCryptoSuites[this.ongoingSessions[request.sessionID].audioCryptoSuite] + ' inline:' + this.ongoingSessions[request.sessionID].audioSRTP.toString('base64'));
        ...
        if (request.type === this.hap.StreamRequestTypes.STOP && typeof this.ongoingSessions[request.sessionID] === 'object') {
        ...
        if (request.type === this.hap.StreamRequestTypes.RECONFIGURE && typeof this.ongoingSessions[request.sessionID] === 'object') {
            // <---- TODO
            //(config.debug.includes(Debugging.CAMERA) || config.debug.includes(Debugging.DOORBELL)) ) && this.log.info('Reconfiguration request for live stream on "%s"', this.deviceData.description);
        }
    ...
    updateServices(deviceData) {
        ...
        if (deviceData.hksv === true && operatingModeService !== undefined) {
            ...
            operatingModeService.updateCharacteristic(this.hap.Characteristic.ManuallyDisabled, (deviceData.streaming_enabled === true ? this.hap.Characteristic.ManuallyDisabled.ENABLED : this.hap.Characteristic.ManuallyDisabled.DISABLED));
            ...
            if (deviceData.has_statusled === true && typeof deviceData.statusled_brightness === 'number') {
                ...
                // 1 = low
                // 2 = high
                operatingModeService.updateCharacteristic(this.hap.Characteristic.CameraOperatingModeIndicator, (deviceData.statusled_brightness !== 1 ? this.hap.Characteristic.CameraOperatingModeIndicator.ENABLE : this.hap.Characteristic.CameraOperatingModeIndicator.DISABLE));
            }
        ...
        if (deviceData.hksv === true && recordingManagementService !== undefined) {
            ...
            recordingManagementService.updateCharacteristic(this.hap.Characteristic.RecordingAudioActive, deviceData.audio_enabled === true ? this.hap.Characteristic.RecordingAudioActive.ENABLE : this.hap.Characteristic.RecordingAudioActive.DISABLE);
        }
    ...
    createCameraMotionServices() {
        ...
        if (this.deviceData.has_motion_detection === true &&
            typeof this.deviceData.activity_zones === 'object') {
    ...
    createCameraHKSVServices() {
        ...
        if (this.deviceData.has_statusled === true) {
            ...
            operatingModeService.getCharacteristic(this.hap.Characteristic.CameraOperatingModeIndicator).onSet((value) => {
                ...
                if ((this.deviceData.statusled_brightness === 1 && value === this.hap.Characteristic.CameraOperatingModeIndicator.ENABLE) ||
                    ...
                    if (this.log.info) {
                        this.log.info('Recording status LED on "%s" was turned', this.deviceData.description, (value === this.hap.Characteristic.CameraOperatingModeIndicator.ENABLE ? 'on' : 'off'));
                    }
                }
            });

            operatingModeService.getCharacteristic(this.hap.Characteristic.CameraOperatingModeIndicator).onGet(() => {
                return (this.deviceData.statusled_brightness !== 1 ? this.hap.Characteristic.CameraOperatingModeIndicator.ENABLE : this.hap.Characteristic.CameraOperatingModeIndicator.DISABLE);
            });
        }
        ...
        if (this.deviceData.has_microphone === true) {
            recordingManagementService.getCharacteristic(this.hap.Characteristic.RecordingAudioActive).onSet((value) => {
                if ((this.deviceData.audio_enabled === false && value === this.hap.Characteristic.RecordingAudioActive.ENABLE) ||
                    ...
                    if (this.log.info) {
                        this.log.info('Audio recording on "%s" was turned', this.deviceData.description, (value === this.hap.Characteristic.RecordingAudioActive.ENABLE ? 'on' : 'off'));
                    }
                }
            });

            recordingManagementService.getCharacteristic(this.hap.Characteristic.RecordingAudioActive).onGet(() => {
                return (this.deviceData.audio_enabled === true ? this.hap.Characteristic.RecordingAudioActive.ENABLE : this.hap.Characteristic.RecordingAudioActive.DISABLE);
            });
        }
        ...
        operatingModeService.getCharacteristic(this.hap.Characteristic.HomeKitCameraActive).onSet((value) => {
            if (value !== operatingModeService.getCharacteristic(this.hap.Characteristic.HomeKitCameraActive).value) {
                ...
                if ((this.deviceData.streaming_enabled === false && value === this.hap.Characteristic.HomeKitCameraActive.ON) ||
                    ...
                    if (this.log.info) {
                        this.log.info('Camera on "%s" was turned', this.deviceData.description, (value === this.hap.Characteristic.HomeKitCameraActive.ON ? 'on' : 'off'));
                    }
                }
        ...
        operatingModeService.getCharacteristic(this.hap.Characteristic.HomeKitCameraActive).onGet(() => {
            return (this.deviceData.streaming_enabled === true ? this.hap.Characteristic.HomeKitCameraActive.ON : this.hap.Characteristic.HomeKitCameraActive.OFF);
        });
    }
}